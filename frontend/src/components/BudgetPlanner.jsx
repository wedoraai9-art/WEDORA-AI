...
export const BudgetPlanner = () => {
  ...
  const calc = async () => {
    ...
    try {
      const r = await estimateBudget({
        total_budget: Number(total),
        ...
        <div className="pearl-card p-6 lg:col-span-2">
          <div className="grid grid-cols-2 gap-4">
            <label className="col-span-2">
              <span className="text-xs uppercase tracking-widest text-[#988FA6]">
                Wedding Budget (₹)
              </span>
              ...
              <input
                ...
                className="w-full mt-2 accent-[#F7B7D8]"
              />
            ...
            <label>
              ...
              <input
                data-testid={BUDGET.guestsInput}
                type="number"
                value={guests}
            ...
            <label className="col-span-2">
              ...
              <input
                data-testid={BUDGET.functionsInput}
                type="number"
                min="1"
                max="7"
                value={functions}
          ...
          <button
            data-testid={BUDGET.calcBtn}
            onClick={calc}
            disabled={loading}
            className="glow-btn w-full mt-5 flex items-center justify-center gap-2 disabled:opacity-60"
          >
        ...
        <div className="lg:col-span-3">
          {!started ? (
            <div className="pearl-card p-10 h-full flex items-center justify-center text-center text-[#6B617A]">
              <div>
                <p>
                  Set your budget above and WEDORA will lay out a beautiful,
                  itemized plan.
                </p>
              </div>
            </div>
            ...
            <div className="pearl-card p-10 h-full flex items-center justify-center text-center text-[#6B617A]">
              <div>
                ...
                <p>WEDORA is building your budget plan…</p>
              </div>
            </div>
          ) : data ? (
            <div data-testid={BUDGET.result} className="pearl-card p-6">
              <div className="flex flex-wrap items-center justify-between mb-5">
                <div>
                  ...
                  <p className="font-display text-3xl text-[#2D2638]">
                    {fmt(data.total)}
                  </p>
                </div>

                <div className="text-right">
                  <p className="text-xs text-[#988FA6]">
                    {data.guest_count} guests • {data.city} •{' '}
                    {data.functions} functions
                  </p>

                  <p className="text-xs text-[#6B617A] mt-1">
                    ~{fmt(data.per_head)} food per head
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-6">
                {data.categories.map((c, i) => (
                  <div
                    key={c.name}
                    className="flex items-center gap-3 rounded-2xl p-3 bg-white/60 border border-white/70"
                  >
                    <span
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-[#2D2638] truncate">
                        {c.name}
                      </p>

                      <p className="text-xs text-[#988FA6] truncate">
                        {c.note}
                      </p>
                    </div>
            ...
            <div className="pearl-card p-10 h-full flex items-center justify-center text-center text-[#6B617A]">
              <div>
                <p>We couldn't build the budget right now. Please try again.</p>
              </div>
            </div>
          )}
        </div>
